#include <stdio.h>
#include <stdlib.h>
#include <string.h>

/*descobri pq quando insere com uma qtd fixa vai uma coisa e quando insere um por um sai outra*/
typedef struct data{ /*lista para as datas/quantidade*/
    int dia;
    int mes;
    int ano;
    int qtd;
    struct data *prox;
}data;

typedef struct estoque{ /*lista para os codigos/produto*/
    int codigo;
    char descricao[40];
    char marca[30];
    struct data *validade;
    struct estoque *prox;
}estoque;

data *inserir_validade(data *validade, int dia, int mes, int ano , int qtd)
{
    data *new = (data*)malloc(sizeof(data));

    if (new == NULL)
    {
        printf("Produto cheio\n");
        return validade;
    }
    else
    {
        if (validade == NULL)
        {
            new->dia = dia;
            new->mes = mes;
            new->ano = ano;
            new->qtd += qtd; /*acrescentando ao que já tinha*/
            new->prox = NULL;
            validade = new;
            return validade;
        }
        else
        {
            data *aux;
            aux = validade;

            while ((aux->prox != NULL) && (aux->dia > dia) && (aux->mes > mes) && (aux->ano > ano))
            {
                aux = aux->prox;
            }

           if (aux->prox == NULL)
            {
                if (aux->ano > ano)
                {
                    new->prox = aux->prox;
                    new->dia = dia;
                    new->mes = mes;
                    new->ano = ano;
                    new->qtd += qtd;
                    aux->prox = new;
                } 
                else if (aux->ano == ano)
                {
                    if (aux->mes > mes)
                    {
                        new->prox = aux->prox;
                        new->ano = ano;
                        new->mes = mes;
                        new->dia = dia;
                        new->qtd += qtd;
                        aux->prox = new;
                    }
                    else if (aux->mes == mes)
                    {
                        if (aux->dia > dia)
                        {
                            new->prox = aux->prox;
                            new->qtd += qtd;
                            new->ano = ano;
                            new->mes = mes;
                            new->dia = dia;
                            aux->prox = new;
                        }
                        else if(aux->dia == dia)
                        {
                            aux->qtd += qtd; /*validades com o msm dia*/
                        }
                        else
                        {
                            new->prox = aux->prox;
                            new->ano = ano;
                            new->mes = mes;
                            new->dia = dia;
                            new->qtd = aux->qtd;

                            aux->prox = new;
                            new->ano = ano;
                            new->mes = mes;
                            new->dia = dia;
                            aux->qtd = qtd;
                        }
                        
                    }
                    else
                    {
                        new->prox = aux->prox;
                        new->ano = ano;
                        new->mes = mes;
                        new->dia = dia;
                        new->qtd = aux->qtd;

                        aux->prox = new;
                        aux->dia = dia;
                        aux->ano = ano;
                        aux->mes = mes;
                        aux->qtd = qtd;
                    }
                    
                }
                else
                {
                    new->prox = aux->prox;
                    aux->dia = dia;
                    aux->ano = ano;
                    aux->mes = mes;
                    new->qtd = aux->qtd;

                    aux->prox = new;
                    aux->dia = dia;
                    aux->ano = ano;
                    aux->mes = mes;
                    aux->qtd = qtd;
                }
                    
                return validade;
            }
            else
            {
                if (aux->ano > ano)
                {
                    new->prox = aux->prox;
                    aux->dia = dia;
                    aux->ano = ano;
                    aux->mes = mes;
                    new->qtd += qtd;
                    aux->prox = new;       
                }
                else if (aux->ano == ano)
                {
                    if (aux->mes > mes)
                    {
                        new->prox = aux->prox;
                        aux->dia = dia;
                        aux->ano = ano;
                        aux->mes = mes;
                        new->qtd += qtd;
                        aux->prox = new;
                    }
                    else if (aux->mes == mes)
                    {
                        if (aux->dia > dia)
                        {
                            new->prox = aux->prox;
                            aux->dia = dia;
                            aux->ano = ano;
                            aux->mes = mes;
                            new->qtd += qtd;
                            aux->prox = new;
                        }
                        else if(aux->dia == dia)
                        {
                            aux->qtd += qtd;
                        }
                        else
                        {
                            new->prox = aux->prox;
                            aux->dia = dia;
                            aux->ano = ano;
                            aux->mes = mes;
                            new->qtd = aux->qtd;
                            aux->prox = new;
                            aux->dia = dia;
                            aux->ano = ano;
                            aux->mes = mes;
                            aux->qtd = qtd;
                        }
                    }
                    else
                    {
                        new->prox = aux->prox;
                        aux->dia = dia;
                        aux->ano = ano;
                        aux->mes = mes;
                        new->qtd= aux->qtd;
                        aux->prox = new;
                        aux->dia = dia;
                        aux->ano = ano;
                        aux->mes = mes;
                        aux->qtd = qtd;
                    }
                }
                else
                {
                    new->prox = aux->prox;
                    aux->dia = dia;
                    aux->ano = ano;
                    aux->mes = mes;
                    new->qtd = aux->qtd;
                    aux->prox = new;
                    aux->dia = dia;
                    aux->ano = ano;
                    aux->mes = mes;
                    aux->qtd = qtd;
                }
                return validade;
            }       
        }

    }
}

estoque *criar_produto(estoque *produto, int cod, int dia, int mes, int ano, int qtd, char des[], char marca[])
{
    estoque *new;
    new = (estoque*) malloc(sizeof(estoque));
    if (new == NULL) printf("Não foi possivel alocar memória\n");
    else
    {   
             
        if (produto == NULL) /*insere 1 elemento*/
        {
            new->codigo = cod;
            strcpy(new->descricao, des);
            strcpy(new->marca, marca);
            new->validade = NULL;
            new->validade = inserir_validade(new->validade, dia, mes, ano, qtd);
            new->prox = NULL;
            produto = new;
            return produto;
        }
        else
        {
            estoque *aux = produto;
            while (aux->prox != NULL && cod > aux->codigo)
            { 
                aux = aux->prox;
            }

            if (aux->prox == NULL) /*O numero a inserir é maior que todos */
            {   
                printf("NULLO\n");
                if (cod > aux->codigo)
                {
                    printf("Aqui\n");
                    new->prox = NULL;
                    new->codigo = cod;
                    strcpy(new->descricao, des);
                    strcpy(new->marca, marca);
                    new->validade = NULL;
                   // new->validade = inserir_validade(new->validade, dia, mes, ano, qtd);
                    aux->prox = new;         
                }
               /* else if (aux->codigo == cod)
                {
                    new->validade = NULL;
                    aux->validade= inserir_validade(new->validade, dia, mes, ano, qtd);
                }
                else
                {
                    new->codigo = aux->codigo; 
                    new->validade = aux->validade;
                    strcpy(new->descricao, aux->descricao);
                    strcpy(new->marca, aux->marca);
                    new->prox = NULL;   
                    aux->codigo = cod;
                    strcpy(aux->marca, marca);
                    strcpy(aux->descricao, des);
                    new->validade = NULL;
                    aux->validade = inserir_validade(new->validade, dia, mes, ano, qtd);
                    aux->prox = new;
                }*/

            }
           else 
            {   
                if (aux->codigo == cod)
                {
                     new->validade = NULL;
                    aux->validade = inserir_validade(new->validade, dia, mes, ano, qtd);
                }
                else{
                    new->codigo = aux->codigo;
                    new->validade = aux->validade;
                    strcpy(new->marca, aux->marca);
                    strcpy(new->descricao, aux->descricao);
                    new->prox = aux->prox;
                    aux->codigo = cod;
                    strcpy(aux->marca, marca);
                    strcpy(aux->descricao, des);
                    new->validade = NULL;
                    aux->validade = inserir_validade(new->validade, dia, mes, ano, qtd);
                    aux->prox = new;
                }          

            }
            return produto; 
        }
    }      
}
estoque *criarlista_encadeada(int i, int n, estoque *produto, estoque *aux, estoque *head)
{
    if (i < n)
    {          
        printf("###########################################\n");
        int cod, dia, mes, ano, qtd;
        char desc[40], marca[30];
        printf("Digite o codigo do produto: ");
        scanf("%d", &cod);
        getchar();
        printf("Digite a descrição do produto: ");
        scanf("%s", desc);
        printf("Digite a marca do produto: ");
        scanf("%s", marca);
        printf("Digite a data de validade do produto em numeros:\n");
        printf("Dia: ");
        scanf("%d", &dia);
        printf("Mês: ");
        scanf("%d", &mes);
        printf("Ano: ");
        scanf("%d", &ano);           
        printf("Digite a quantidade do produto: ");
        scanf("%d", &qtd);
        if (head == NULL) /*colocar o elemento como 1*/
        {
            head  = criar_produto(produto, cod, dia, mes, ano, qtd, desc, marca);  
            head -> prox = NULL;
            produto = head;
        }
        else
        {
            aux = head;
            while(aux->prox != NULL)
                aux = aux->prox;

            aux->prox  = criar_produto(produto, cod, dia, mes, ano, qtd, desc, marca);         
        }    
        
        return criarlista_encadeada(i+1, n,produto,aux, head);
    }  
    else return head;
}

int quantidade(estoque *head, int i) /*função para ver quantos produtos tem*/
{
    if (head != NULL)
    {
        i++;
        head = head->prox;
        quantidade(head, i);
    }
    else return i;      
}

void imprimir(estoque *aux)
{ 
    if (aux == NULL) return;
    else
    {
        printf("-----------------------------------------\n");
        printf("Código: %d\n", aux->codigo);
        printf("Descrição: %s\n", aux->descricao);
        printf("Marca: %s\n", aux->descricao);
        data *data_aux = aux->validade;
        while (data_aux != NULL)
        { 
            printf("*****************************************\n");
            printf("Validade: %d/%d/%d\n", data_aux->dia, data_aux->mes, data_aux->ano);
            printf("Quantidade: %d\n", data_aux->qtd);
            data_aux = data_aux->prox;
            printf("*****************************************\n");
        }
        printf("-----------------------------------------\n");
        aux= aux->prox;
        imprimir(aux);
    }

}

/*estoque *retirando(estoque *head, estoque *aux, estoque *menor)
{
   
}*/

int menu(estoque *head,  estoque *elem, estoque *aux, int o)
{
    printf("-------------------- Sistema de gerenciamento de estoque --------------------\n");
    printf("\n----------------- Digite a operação que deseja realizar -------------------\n");
    printf("[0] Finalizar\n[1] Criar o estoque\n[2] Retirar produtos\n[3] Consultar produtos\n[4] Total de produtos\n");
    scanf("%d", &o);
    
    if (o != 0)
    {  
        printf("Por favor aguarde.....\n\n");
        if (o == 1)
        {
            int n;
            estoque *aux;
            printf("Insira quantos produtos deseja inserir: ");
            scanf("%d", &n);
            aux = criarlista_encadeada(0, n,elem, aux, head);    
            head = aux;
            printf("------------ PRODUTO(s) INSERIDO(s) ------------\n\n\n");                      
        }
        else if (o == 2)
        {
            int qtd_retira, cod_retirar;
           
            printf("------------ PRODUTO RETIRADO ---------\n");
        }
        else if (o == 3)
        {
            estoque *aux = head;
            imprimir(aux);
        }
        else if (o == 4)
        {
            int q = quantidade(head, 0);
            printf("Existe %d produto(s)\n", q);
        }
        return menu(head, elem, aux,  o);
    }else printf("Finalizando....\n");
}
int main()
{
    estoque *head = NULL, *elem = NULL, *aux = NULL;
  
    int o = 0; 
  
    menu(head, elem, aux, o);

    return 0;
}
